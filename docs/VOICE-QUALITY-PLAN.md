...
| A.1 EOU qisqartirish            | ⏳ o'lchov kutmoqda | `SmartTurnDetector.isConfidentlyComplete` + `SpeechGate.setEarlyClose`. **Butun `turn.*` bloki `stt.vad-gating.enabled: true` ni talab qiladi** — u o'chiq bo'lsa gate qurilmaydi va Smart Turn hech qachon chaqirilmaydi; nomuvofiq sozlama startda WARN beradi. Provayderda qoladimi yoki gate'ga ko'chadimi — qaror `docs/REGRESSION.md` §4 dagi ikki o'lchovga bog'liq (`scripts/replay-sweep.sh` + jonli `stage=eou`/`stage=stt_final`). Korpus kelmaguncha ochiq |
| A.2 Speculative TTS             | ✅             | Guess'ning birinchi jumlasi `TtsCache` ga oldindan sintez qilinadi; `dialog.preemptive-tts: false`                            |
