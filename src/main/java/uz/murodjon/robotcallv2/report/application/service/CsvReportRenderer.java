...
public class CsvReportRenderer {
    ...
    /** The call list behind {@code POST /api/reports/calls/export}, one row per call. */
    public byte[] renderCalls(List<CallRow> rows) {
    ...
    /** The {@code GET /api/reports/export?format=csv} content type. */
    public String contentType() {
        return CSV;
    }

    /**
     * The whole report bundled as one file for {@code GET /api/reports/export?format=csv}
     * (§10.10) — its four sections stacked with a blank line between them, since a CSV
     * has no notion of separate sheets.
     */
