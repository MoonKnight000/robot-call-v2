...
public class GoogleSttProvider implements SttProvider {
    ...
    public SttSession startStream(String languageCode, List<String> alternativeLanguages,
        ...
        // The languages this caller might answer in instead. Google recognizes the audio
        // in whichever of them fits best and says which it chose; the cap of three is
        // its own limit on the field.
        if (alternativeLanguages != null && !alternativeLanguages.isEmpty()) {
            recConfig.addAllAlternativeLanguageCodes(alternativeLanguages.stream()
                    .filter(l -> l != null && !l.isBlank() && !l.equalsIgnoreCase(languageCode))
                    .limit(3)
                    .toList());
        }
