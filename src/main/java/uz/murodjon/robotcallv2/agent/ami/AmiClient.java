package uz.murodjon.robotcallv2.agent.ami;
...
public class AmiClient {
    ...
    public String executeCommand(String command) {
        ...
        try (Socket socket = new Socket()) {
            ...
            send(out, "Action: Command\r\nCommand: " + command + "\r\n\r\n");
            String cmdResponse = readCommandResponse(in);
    ...
    private static String readCommandResponse(BufferedReader in) throws IOException {
        StringBuilder sb = new StringBuilder();
        String line;
        boolean inFollows = false;
        while ((line = in.readLine()) != null) {
            if (line.startsWith("Response: Follows")) {
                inFollows = true;
                continue;
            }
            ...
            if (!inFollows && line.isEmpty()) {
                break;
            }
            if (inFollows && !line.startsWith("Privilege: Command")) {
                sb.append(line).append('\n');
            }
        }
        return sb.toString();
