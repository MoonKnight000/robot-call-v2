...
import uz.murodjon.uysotvoice.notification.config.NotificationProperties;
...
/**
 * Delivers one notification event via the Telegram Bot API's {@code sendMessage} (§11
 * settings) — one bot for the whole deployment ({@link NotificationProperties#telegramBotToken}),
 * a company only supplies the destination {@code chat_id}. Swallow-and-log on failure,
 * same posture as every other channel sender here.
 */
@Component
