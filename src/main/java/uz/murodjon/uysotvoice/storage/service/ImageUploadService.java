...
public class ImageUploadService {
    ...
    public StoredFile upload(MultipartFile file, long companyId) {
        ...
        if (file.getSize() > MAX_BYTES) {
            throw new ValidationException("image must be at most 5 MB");
        }
        if (!ALLOWED_CONTENT_TYPES.contains(file.getContentType())) {
            throw new ValidationException(
                    "unsupported image type '" + file.getContentType() + "'; allowed: " + ALLOWED_CONTENT_TYPES);
        }
        byte[] data;
        try {
            data = file.getBytes();
        ...
        if (stored == null) {
            throw new ExternalServiceException("object-storage", "image upload failed — storage unavailable");
        }
        return stored;
    }
}
